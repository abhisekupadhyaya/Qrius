...
def app_calib_im(master):
    oAppCalibIM = AppCalibIM(master)
    return oAppCalibIM

class AppCalibIM:
    def __init__(self, master):
        self.master = master
        self.master.title('Current Measure Calibration')
        self.CMRange = CURRENT_RANGE_LABELS
        ...
        self.Current_labels = []
        self.CMRange_selected = IntVar()
        self.data_point = IntVar()
        self._vCreateWidgets()
        self.vDisableCMRangeSelect()
    ...
    def _vCreateWidgets(self):
        ...
        for rows in range(max_rows):
            for columns in range(max_columns):
                if (rows*max_columns + columns <= max_range-1):
                    self.RBCMRange.append(Radiobutton(self.Frames[1],variable = self.CMRange_selected,value=(columns + rows*max_columns)))
                    if ((columns + rows*max_columns) not in DISABLED_RANGE):
                        ...
                        self.RBCMRange[columns + rows*max_columns].grid(row=shifted_rows,column=shifted_columns*3)
                        self.RBCMRange[columns + rows*max_columns].deselect()
                        Label(self.Frames[1], text=self.CMRange[columns + rows*max_columns], fg='blue').grid(row=shifted_rows,column=(shifted_columns*3)+1, sticky=W)
        ...
        for row in range(NO_OF_POINTS):
            ...
            self.EntryADC.append(Entry(self.Frames[2],bg='white',justify=LEFT,width=8,textvariable=self.entryADCValues[row],readonlybackground='grey',takefocus=False))
            self.EntryADC[row].grid(row=row+1, column=2, sticky=W)
            ## Actual Current ##
            self.entryCurrentValues.append(DoubleVar())
            self.EntryCurrent.append(Entry(self.Frames[2], bg='white', justify=LEFT, width=8, textvariable=self.entryCurrentValues[row]))
    ...
    def vEnableCMRangeSelect(self):
        '''
        Enables Range Selection
        '''
        for column in range(CURRENT_RANGE):
            self.RBCMRange[column].config(state = NORMAL)
            self.RBCMRange[column].deselect()
        return

    def vDisableCMRangeSelect(self):
        '''
        Disables Range Selection
        '''
        for column in range(CURRENT_RANGE):
            self.RBCMRange[column].config(state = DISABLED)
            self.RBCMRange[column].deselect()
        return
    ...
    def vUpdateCurrentUnit(self):
        ...
        '''
        for row in range(NO_OF_POINTS):
            self.Current_labels[row].config(text=self.CurrentUnit[self.CMRange_selected.get()])
        return
